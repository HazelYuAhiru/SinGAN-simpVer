            ...
            for G, Z_opt, real_curr, real_next, noise_amp in zip(Gs, Zs, reals, reals[1:], NoiseAmp):
                if count == 0:
                    ...
                    z = functions.generate_noise(
                        [opt.nc_z, Z_opt.shape[2] - 2 * pad_noise, Z_opt.shape[3] - 2 * pad_noise], device=device)
                z = m_noise(z)
...
def train_paint(opt, Gs, Zs, reals, NoiseAmp, centers, paint_inject_scale):
    in_s = torch.full(reals[0].shape, 0, device=device)
    scale_num = 0
    nfc_prev = 0
...
def init_models(opt):
    ...
    netD = models.WDiscriminator(opt).to(device)
